            ...
            try {
                ...
                try {
                    ...
                    DebugE93bdbLog.log("MediaButtonRegistrar.ensureRegistered",
                            "MEDIA_BUTTON registered", "H2", d);
                } catch (Exception ignored) {}
                // #endregion
                long tookMs = SystemClock.uptimeMillis() - t0;
                if (tookMs >= 8L) {
                    try {
                        JSONObject d = new JSONObject();
                        d.put("tookMs", tookMs);
                        d.put("registered", registered);
                        DebugSessionLog.logAlways("MediaButtonRegistrar.ensureRegistered",
                                "latency sample", "H-LAT-REGISTER", d);
                    } catch (Exception ignored) {}
                }
            } catch (Exception e) {
                registered = false;
