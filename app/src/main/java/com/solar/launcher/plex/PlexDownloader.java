...
/**
 * 2026-07-06: Download Plex album tracks to local Plex/Artist/Album folder.
 */
public final class PlexDownloader {
    ...
    private PlexDownloader() {}

    public static void downloadAlbum(final Context ctx, final PlexArtist artist,
            final PlexAlbum album, final List<PlexSong> songs, final Callback cb) {
        executor.execute(new Runnable() {
            @Override public void run() {
                ...
                File root = new File(DeviceFeatures.getNewMediaRoot(ctx), "Plex");
                ...
                for (int i = 0; i < songs.size(); i++) {
                    ...
                    try {
                        ...
                        if (!out.exists() || out.length() < 1024) {
                            downloadUrl(PlexClient.getInstance().getDownloadUrl(s), out);
                        }
                        saved++;
                        final int done = i + 1;
    ...
    /** 2026-07-06: Same TLS/HTTP stack as PlexClient — avoids refused/wrong-port downloads. */
