...
public class ModulesFragment extends ListFragment implements ModuleListener {
	...
	public void onActivityCreated(Bundle savedInstanceState) {
		...
		// Y1/Y2: first row was clipped under status bar — pad list below system chrome.
		int topPad = eightDp + statusBarPaddingPx();
		getListView().setDivider(null);
		getListView().setDividerHeight(sixDp);
		getListView().setPadding(eightDp, topPad, eightDp, eightDp);
		getListView().setClipToPadding(false);
	}

	// Status bar height so module rows clear the clock on iPod-scale displays.
	private int statusBarPaddingPx() {
	...
	public void onListItemClick(ListView l, View v, int position, long id) {
		...
		InstalledModule module = mModuleUtil.getModule(packageName);
		if (module == null)
			return;

		// Y1/Y2 wheel: center toggles enabled — checkbox is not focusable and sits under status bar.
		if (!canToggleModule(module)) {
			Toast.makeText(getActivity(), getString(R.string.no_min_version_specified), Toast.LENGTH_SHORT).show();
			return;
		}

		boolean enabled = !mModuleUtil.isModuleEnabled(packageName);
		mModuleUtil.setModuleEnabled(packageName, enabled);
		mModuleUtil.updateModulesList(true);
		reloadModules.run();
	}

	private boolean canToggleModule(InstalledModule item) {
		if (item.minVersion == 0)
			return false;
		if (installedXposedVersion != 0 && item.minVersion > installedXposedVersion)
			return false;
		if (item.minVersion < ModuleUtil.MIN_MODULE_VERSION)
			return false;
		return true;
	}
	...
	private class ModuleAdapter extends ArrayAdapter<InstalledModule> {
		...
		public View getView(int position, View convertView, ViewGroup parent) {
			...
			if (convertView == null) {
				// Checkbox is display-only on Solar hardware; keep listener for touch devices.
			...
			boolean enabled = mModuleUtil.isModuleEnabled(item.packageName);
			title.setText((enabled ? "\u2713 " : "\u2717 ") + item.getAppName());
