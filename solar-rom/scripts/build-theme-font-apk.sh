#!/usr/bin/env bash
# Build the Solar Theme Font Xposed module APK (API 17+ — Y1 and Y2).
# Output: solar-rom/vendor/xposed/solar-theme-font/SolarThemeFont.apk
set -euo pipefail

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
MOD_DIR="$SCRIPT_DIR/../vendor/xposed/solar-theme-font"
BUILD_DIR="$MOD_DIR/build"
OUT_APK="$MOD_DIR/SolarThemeFont.apk"
MIN_API=17

die() { echo "build-theme-font-apk: $*" >&2; exit 1; }
...
public final class XposedHelpers {
    ...
    public static Object findAndHookMethod(String className, ClassLoader classLoader, String methodName, Object... parameterTypesAndCallback) { return null; }
}
EOF
...
EOF
cat > "$BUILD_DIR/stubsrc/de/robv/android/xposed/IXposedHookInitZygote.java" <<'EOF'
package de.robv.android.xposed;
public interface IXposedHookInitZygote {
    class StartupParam {
...
EOF
cat > "$BUILD_DIR/stubsrc/de/robv/android/xposed/callbacks/XC_LayoutInflated.java" <<'EOF'
package de.robv.android.xposed.callbacks;
...
public abstract class XC_LayoutInflated {
    ...
    protected abstract void handleLayoutInflated(LayoutInflatedParam liparam) throws Throwable;
}
EOF
...
echo "==> javac module sources (min API $MIN_API)"
