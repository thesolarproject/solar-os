#!/usr/bin/env bash
# After a main-branch release: bump versionName +0.1 and versionCode +1 in app/build.gradle.
set -euo pipefail
...
current = Decimal(name_m.group(1))
next_name = format(current + Decimal("0.1"), "f").rstrip("0").rstrip(".")
if "." not in next_name:
    next_name += ".0"
next_code = int(code_m.group(1)) + 1
