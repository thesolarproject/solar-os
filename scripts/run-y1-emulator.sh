#!/usr/bin/env bash
# 2026-07-11 — Launch API-17 AVD pinned as Y1 (480×360) for Solar volume/HUD + input parity.
# Layman: starts a Y1-sized emulator so Solar volume menus work without real hardware.
# Tech: reuses A5 AVD machinery with different lcd size + persist.solar.device_family=y1.
# Usage: ./scripts/run-y1-emulator.sh [--install] [--no-window]
set -euo pipefail
ROOT="$(cd "$(dirname "$0")/.." && pwd)"
# shellcheck source=/dev/null
[[ -f "$ROOT/scripts/env.sh" ]] && source "$ROOT/scripts/env.sh" || true
SDK="${ANDROID_HOME:-${ANDROID_SDK_ROOT:-$HOME/Android/Sdk}}"
AVD_NAME="${SOLAR_Y1_AVD:-solar_y1_480x360}"
...
if ! "$EMU" -list-avds 2>/dev/null | grep -qx "$AVD_NAME"; then
  echo "Creating AVD $AVD_NAME (480×360, API 17)…"
  if [[ ! -x "$AVDM" ]]; then
...
if ! "$ADB" devices 2>/dev/null | grep -q emulator; then
  ...
  "$EMU" "${EMU_ARGS[@]}" >/tmp/solar-y1-emulator.log 2>&1 &
  "$ADB" wait-for-device
...
"$ADB" shell setprop persist.solar.device_family y1 2>/dev/null || true
...
if [[ "$INSTALL" -eq 1 ]]; then
  ...
  adb shell setprop persist.solar.device_family y1 || true
  adb shell am force-stop com.solar.launcher || true
  adb shell am start -n com.solar.launcher/.MainActivity || true
fi

echo "Y1 emulator ready. Volume keys / -/= should open Solar volume HUD."
echo "Log: /tmp/solar-y1-emulator.log"
